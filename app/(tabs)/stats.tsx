...
export default function StatsScreen() {
  ...
  const DonutChart = () => (
    <View style={[s.donutWrapper, { width: DONUT_SIZE, height: DONUT_SIZE }]}>
      ...
      <View style={s.donutCenter}>
        {grandTotal > 0 ? (
          <>
            <Text style={[s.donutAmount, { color: theme.text }]}>
              ${totalPayments.toFixed(0)}
            </Text>
            <Text style={s.donutLabel}>pagado</Text>
          </>
        ) : (
          <Text style={s.donutLabel}>Sin datos</Text>
        )}
      </View>
    </View>
  );
...
const styles = (theme: any) =>
  StyleSheet.create({
    ...
    donutHole: {
      position: "absolute",
      width: DONUT_SIZE - STROKE * 2,
      height: DONUT_SIZE - STROKE * 2,
      borderRadius: (DONUT_SIZE - STROKE * 2) / 2,
    },
    donutCenter: { alignItems: "center" },
    donutAmount: { fontSize: 28, fontWeight: "800" },
    donutLabel: { fontSize: 13, color: TEXT_SECONDARY },
