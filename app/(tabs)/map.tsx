...
export default function MapScreen() {
  ...
  const isDark = mode === "dark" || (mode === "normal" && theme.background === "#0f172a");
  ...
  const handleLongPress = async (lat: number, lng: number) => {
    ...
    const defaultLabel = place?.street
  ...
  const filteredMarkers = filterCategory ? markers.filter((m) => m.category === filterCategory) : markers;
    ...
    <KeyboardAvoidingView style={s.container} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={s.header}>
        ...
        <View style={s.headerRight}>
          <TouchableOpacity style={s.iconBtn} onPress={() => { setShowSearch(!showSearch); setSearchResults([]); setSearchQuery(""); }}>
      ...
      <View style={s.filterRow}>
        <TouchableOpacity style={[s.filterChip, filterCategory === null && { backgroundColor: PRIMARY }]} onPress={() => setFilterCategory(null)}>
          <Text style={[s.filterChipText, filterCategory === null && { color: "#fff" }]}>Todos</Text>
        </TouchableOpacity>
        {CATEGORIES.map((cat) => {
          const Icon = cat.icon;
          const active = filterCategory === cat.key;
          return (
            <TouchableOpacity key={cat.key} style={[s.filterChip, active && { backgroundColor: cat.color }]} onPress={() => setFilterCategory(active ? null : cat.key)}>
              <Icon size={12} color={active ? "#fff" : TEXT_SECONDARY} />
              <Text style={[s.filterChipText, active && { color: "#fff" }]}>{cat.label}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
      ...
      {showSearch && (
        <View style={s.searchContainer}>
          <View style={s.searchRow}>
            <TextInput style={s.searchInput} placeholder="Buscar lugar..." placeholderTextColor={TEXT_SECONDARY} value={searchQuery} onChangeText={setSearchQuery} onSubmitEditing={handleSearch} returnKeyType="search" autoFocus />
            {searchQuery.length > 0 && <TouchableOpacity onPress={() => { setSearchQuery(""); setSearchResults([]); }}><X size={18} color={TEXT_SECONDARY} /></TouchableOpacity>}
          </View>
          {searching && <ActivityIndicator size="small" color={PRIMARY} style={{ marginTop: 8 }} />}
          {searchResults.length > 0 && (
            <FlatList data={searchResults} keyExtractor={(_, i) => i.toString()} style={s.resultsList} keyboardShouldPersistTaps="handled"
              renderItem={({ item }) => (
      ...
      <MapView ref={mapRef} style={s.map} provider={PROVIDER_GOOGLE} customMapStyle={isDark ? DARK_MAP_STYLE : []}
        ...
        onLongPress={(e) => { const { latitude, longitude } = e.nativeEvent.coordinate; handleLongPress(latitude, longitude); }}
      >
        {filteredMarkers.map((m) => (
          <Marker key={m.id} coordinate={{ latitude: m.lat, longitude: m.lng }} title={m.label} pinColor={getCategoryColor(m.category)} />
        ))}
      </MapView>

      {filteredMarkers.length > 0 && (
        <View style={s.markersList}>
          <FlatList data={filteredMarkers} keyExtractor={(m) => m.id} horizontal showsHorizontalScrollIndicator={false}
            ...
            renderItem={({ item }) => (
              <View style={[s.markerChip, { borderLeftColor: getCategoryColor(item.category), borderLeftWidth: 3 }]}>
                ...
                <TouchableOpacity onPress={() => deleteMarker(item.id)}><Trash2 size={12} color={TEXT_SECONDARY} /></TouchableOpacity>
              </View>
            )}
          />
        </View>
      ...
      <Modal visible={pendingCoords !== null} transparent animationType="slide" onRequestClose={() => setPendingCoords(null)}>
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
          <View style={s.modalOverlay}>
            <View style={[s.modalCard, { backgroundColor: theme.card }]}>
              ...
              <TextInput style={[s.modalInput, { backgroundColor: theme.background, color: theme.text }]} placeholder="Nombre del lugar" placeholderTextColor={TEXT_SECONDARY} value={modalName} onChangeText={setModalName} autoFocus />
              ...
              <View style={s.categoryRow}>
                {CATEGORIES.map((cat) => {
                  ...
                  return (
                    <TouchableOpacity key={cat.key} style={[s.categoryBtn, active && { backgroundColor: cat.color }]} onPress={() => setModalCategory(cat.key)}>
                      <Icon size={16} color={active ? "#fff" : TEXT_SECONDARY} />
                      <Text style={[s.categoryBtnText, active && { color: "#fff" }]}>{cat.label}</Text>
                    </TouchableOpacity>
                  );
                })}
              </View>
              <View style={s.modalActions}>
                <TouchableOpacity style={[s.modalBtn, { backgroundColor: theme.background }]} onPress={() => setPendingCoords(null)}>
                  <Text style={[s.modalBtnText, { color: theme.text }]}>Cancelar</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[s.modalBtn, { backgroundColor: PRIMARY }]} onPress={confirmAddMarker}>
...
const styles = (theme: any) =>
  StyleSheet.create({
    ...
    header: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 20, paddingTop: 60, paddingBottom: 14, backgroundColor: theme.background },
    headerLeft: { flexDirection: "row", alignItems: "center", gap: 8 },
    headerRight: { flexDirection: "row", gap: 8 },
    cityName: { fontSize: 20, fontWeight: "800", color: theme.text },
    iconBtn: { width: 36, height: 36, borderRadius: 10, backgroundColor: theme.card, justifyContent: "center", alignItems: "center" },
    filterRow: { flexDirection: "row", gap: 8, paddingHorizontal: 16, paddingBottom: 10, backgroundColor: theme.background },
    filterChip: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, backgroundColor: theme.card },
    filterChipText: { fontSize: 12, fontWeight: "600", color: TEXT_SECONDARY },
    searchContainer: { paddingHorizontal: 16, paddingBottom: 8, backgroundColor: theme.background },
    searchRow: { flexDirection: "row", alignItems: "center", backgroundColor: theme.card, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 10, gap: 8 },
    ...
    resultItem: { flexDirection: "row", alignItems: "flex-start", gap: 8, padding: 12, borderBottomWidth: 1, borderBottomColor: theme.background },
    resultText: { flex: 1, fontSize: 13, color: theme.text },
    map: { flex: 1 },
    markersList: { paddingVertical: 12, backgroundColor: theme.background },
    markerChip: { flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: theme.card, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20, maxWidth: 180 },
    ...
    retryBtn: { flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: PRIMARY, paddingHorizontal: 20, paddingVertical: 12, borderRadius: 14, marginTop: 8 },
    ...
    categoryBtn: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, paddingVertical: 10, borderRadius: 12, backgroundColor: theme.background },
